          ...
          <div ref={tableRef} className="h-full overflow-auto custom-scrollbar rounded-2xl border border-slate-200 bg-white shadow-sm">
            <table className="min-w-full border-separate border-spacing-0 text-sm">
              ...
              <thead className="sticky top-0 z-20 bg-white shadow-sm">
                ...
                {/* Feature key row */}
 {/* ───────────────── FEATURE HEADER ROW (UPDATED) ───────────────── */}
<tr className="bg-white sticky top-[30px] z-20">
  {visibleModules.map(mod =>
    visibleGrouped[mod].map(f => (
      <th
        key={f.featureKey}
        title={f.featureName}
        className="border-b border-l border-slate-200 px-2 py-2 text-center min-w-[120px]"
      >
        <div className="flex flex-col items-center leading-tight">
          
          {/* Module name (small grey text) */}
          <span className="text-[9px] font-black uppercase tracking-wider text-slate-400">
            {mod}
          </span>

          {/* Real Feature/Menu name */}
          <span className="text-[11px] font-bold text-slate-700 truncate max-w-[110px]">
            {getFeatureDisplayName(f.featureName, f.featureKey)}
          </span>
  ...
  {/* Row Actions Header */}
  <th className="border-b border-l border-slate-200 px-3 py-2 text-center bg-white sticky right-0 z-30">
    <span className="text-[9px] font-black uppercase tracking-wider text-slate-400">
      Row
    </span>
  </th>
